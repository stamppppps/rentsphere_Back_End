...
async function ensurePermissionCatalog(modules) {
    await Promise.all(modules.map((m) => prisma.permissionCatalog.upsert({
        ...
        update: { isActive: true, module: m },
    })));
}
async function assertOwnerCondoOrThrow(ownerId, condoId) {
    const condo = await prisma.condo.findFirst({
        where: { id: condoId, ownerUserId: ownerId },
        select: { id: true, nameTh: true },
    });
    if (!condo) {
        const err = new Error("Forbidden (not your condo)");
        err.status = 403;
        throw err;
    }
    return condo;
...
async function getAllowedModulesForMembership(membershipId) {
...
async function replaceMembershipOverrides(membershipId, modules) {
...
function asRequiredInt(v) {
    const n = asOptionalInt(v);
    return n === null ? null : n;
}
function asOptionalMoneyNumber(v) {
    if (v === undefined || v === null || String(v).trim() === "")
        return null;
...
function asRequiredTrimmedString(v) {
    const s = asTrimmedString(v);
    return s ? s : null;
}
function asBankCode(v) {
    const s = asTrimmedString(v);
    if (!s)
        return null;
...
/* =========================
   ...
   ========================= */
async function assertOwnerCondo(ownerId, condoId) {
    const condo = await prisma.condo.findFirst({
        where: { id: condoId, ownerUserId: ownerId },
        select: { id: true, nameTh: true },
    });
...
/* =========================
   ...
   ========================= */
async function assertOwnerRoomOrThrow(req, roomId) {
    ...
    if (!room) {
        const err = new Error("Forbidden (not your room)");
        err.status = 403;
        throw err;
    }
    return room;
...
router.post("/condos/:condoId/logo", uploadMemory.single("logo"), async (req, res) => {
    try {
        ...
        if (!req.file)
            return res.status(400).json({ error: "Missing file field 'logo'" });
        const file = req.file;
        ...
        if (!fileUrl)
            return res.status(500).json({ error: "Cloudinary upload failed (no url)" });
...
router.post("/condos", async (req, res) => {
    try {
        ...
        if (!owner)
            return res.status(400).json({ error: "Owner user not found in DB" });
        ...
        if (err?.code === "P2002")
            return res.status(409).json({ error: "Duplicate unique value" });
        if (err?.code === "P2003")
        ...
        return res.json({
            ...
            billingSetting: condo.billingSetting
                ? {
                    ...
                    finePerDay: condo.billingSetting.finePerDay != null
...
router.get("/condos/:condoId/floor-config", async (req, res) => {
    try {
        ...
        if (!floors.length)
            return res.json({ floorCount: 0, roomsPerFloor: [], totalRooms: 0 });
...
router.put("/condos/:condoId/floor-config", async (req, res) => {
    try {
        ...
        const roomsPerFloor = roomsPerFloorRaw.map((x) => Number(x));
...
router.post("/condos/:condoId/invoices", async (req, res) => {
    try {
        ...
        const condoId = String(req.params.condoId);
        await assertOwnerCondoOrThrow(ownerId, condoId);
        const { roomId, totalAmount, status, note, billingMonth, items, } = req.body;
        if (!roomId) {
        ...
        const normalizedBillingMonth = new Date(rawBillingMonth.getFullYear(), rawBillingMonth.getMonth(), 1);
        ...
        const invoiceNo = `INV-${normalizedBillingMonth.getFullYear()}${String(normalizedBillingMonth.getMonth() + 1).padStart(2, "0")}-${roomId.slice(-6).toUpperCase()}-${Date.now()
            .toString(36)
            .toUpperCase()}`;
        const dueDate = new Date(normalizedBillingMonth.getFullYear(), normalizedBillingMonth.getMonth(), 28);
...
router.post("/condos/:condoId/invoices/:invoiceId/notify", async (req, res) => {
    try {
        ...
        if (!token)
            return res.status(500).json({ error: "LINE token ไม่ได้ตั้งค่า" });
...
router.delete("/rooms/:roomId/tenant", async (req, res) => {
    try {
...
router.get("/rooms/:roomId/access-codes", async (req, res) => {
    try {
...
router.post("/rooms/:roomId/access-codes", async (req, res) => {
    try {
        const ownerId = req.user?.id;
        const room = await assertOwnerRoomOrThrow(req, String(req.params.roomId));
        ...
        const expiresInDays = Number.isFinite(daysRaw) && daysRaw > 0 && daysRaw <= 365
            ? Math.trunc(daysRaw)
            : null;
...
/* =========================
   ...
   ========================= */
router.patch("/rooms/:roomId/access-codes/:codeId/disable", async (req, res) => {
    try {
        const room = await assertOwnerRoomOrThrow(req, String(req.params.roomId));
...
/* =========================
   ...
   ========================= */
router.delete("/rooms/:roomId/access-codes/:codeId", async (req, res) => {
    try {
        const room = await assertOwnerRoomOrThrow(req, String(req.params.roomId));
        const codeId = String(req.params.codeId);
        const code = await prisma.tenantRoomCode.findFirst({
...
/* =========================
   ...
   ========================= */
router.get("/rooms/:roomId/contract-detail", async (req, res) => {
    try {
        const room = await assertOwnerRoomOrThrow(req, String(req.params.roomId));
...
/* =========================
   ...
   ========================= */
router.post("/rooms/:roomId/terminate-contract", async (req, res) => {
    try {
        const room = await assertOwnerRoomOrThrow(req, String(req.params.roomId));
        await prisma.$transaction(async (tx) => {
...
/* =========================
   ...
   ========================= */
router.post("/rooms/:roomId/contracts", async (req, res) => {
    try {
        const ownerId = req.user?.id;
        const room = await assertOwnerRoomOrThrow(req, String(req.params.roomId));
        const body = req.body ?? {};
...
/* =========================
   ...
   ========================= */
router.post("/rooms/:roomId/advance-payment", async (req, res) => {
    try {
        const ownerId = req.user?.id;
        const room = await assertOwnerRoomOrThrow(req, String(req.params.roomId));
        const body = req.body ?? {};
...
router.patch("/condos/:condoId/rooms/bulk-price", async (req, res) => {
    try {
        ...
        const roomIds = roomIdsRaw.map((x) => String(x)).filter(Boolean);
        let priceNum = 0;
...
router.patch("/condos/:condoId/rooms/bulk-occupancy", async (req, res) => {
    try {
        ...
        const updated = await prisma.room.updateMany({
            where: { condoId, id: { in: roomIds } },
            data: { occupancyStatus: occupancyStatusRaw },
        });
...
router.post("/condos/:condoId/rooms", async (req, res) => {
    try {
        ...
        if (floorNum === null)
            return res.status(400).json({ error: "floor is required (number)" });
        if (floorNum < 1 || floorNum > 100)
            return res.status(400).json({ error: "floor must be 1-100" });
        // AUTO MODE
        ...
        if (isAuto) {
            ...
            if (count >= 50)
                return res.status(400).json({ error: "Max 50 rooms per floor" });
        ...
        if (!roomNo)
            return res.status(400).json({ error: "roomNo (or number) is required (string)" });
        const rentNum = asOptionalMoneyNumber(rentRaw);
        if (rentNum === null)
            return res.status(400).json({ error: "rentPrice (or price) is required" });
        ...
        if (err?.code === "P2002")
            return res.status(409).json({ error: "Room already exists (unique)" });
...
router.get("/condos/:condoId/rooms", async (req, res) => {
    try {
        ...
        return res.json(rooms.map((r) => {
            const serviceIds = Array.from(new Set((r.extraChargeAssignments ?? []).map((x) => x.serviceId).filter(Boolean)));
            return {
                id: r.id,
                floor: r.floor,
                roomNo: r.roomNo,
...
router.get("/rooms/:roomId/contracts", async (req, res) => {
    try {
        const room = await assertOwnerRoomOrThrow(req, String(req.params.roomId));
...
router.patch("/contracts/:contractId", async (req, res) => {
    try {
        ...
        if (req.body?.moveOutDate)
            data.moveOutDate = new Date(req.body.moveOutDate);
        if (req.body?.status)
            data.status = req.body.status;
        if (req.body?.monthlyRent != null) {
            const n = Number(String(req.body.monthlyRent).replace(/,/g, ""));
            if (!Number.isFinite(n) || n < 0)
                return res.status(400).json({ error: "monthlyRent invalid" });
        ...
        if (req.body?.securityDeposit != null) {
            ...
            if (!Number.isFinite(n) || n < 0)
                return res.status(400).json({ error: "securityDeposit invalid" });
...
router.post("/condos/:condoId/rooms/generate", async (req, res) => {
    try {
        ...
        if (!floors.length)
            return res.status(400).json({ error: "Missing floor-config (Step4)" });
        const data = [];
...
router.patch("/condos/:condoId/rooms/:roomId", async (req, res) => {
    try {
        ...
        if (err?.code === "P2002")
            return res.status(409).json({ error: "Duplicate roomNo" });
...
router.post("/condos/:condoId/services", async (req, res) => {
    try {
        ...
        if (err?.code === "P2002")
            return res.status(409).json({ error: "Duplicate service" });
...
router.post("/condos/:condoId/utilities", async (req, res) => {
    try {
        ...
        if (!okUtility)
            return res.status(400).json({ error: "utilityType must be WATER|ELECTRIC" });
        if (!okBilling)
            return res.status(400).json({ error: "billingType must be METER|METER_MIN|FLAT" });
        if (!Number.isFinite(rate) || rate < 0)
            return res.status(400).json({ error: "rate must be >= 0" });
        const saved = await prisma.condoUtilitySetting.upsert({
            where: { condoId_utilityType: { condoId, utilityType: utilityType } },
            update: {
...
router.post("/condos/:condoId/bank-accounts", async (req, res) => {
    try {
        ...
        if (count >= 2)
            return res.status(400).json({ error: "Max 2 bank accounts" });
        ...
        if (err?.code === "P2002")
            return res.status(409).json({ error: "Duplicate bank account" });
...
router.get("/condos/:condoId/staff", async (req, res) => {
    ...
    const condoId = String(req.params.condoId);
    try {
        await ensurePermissionCatalog(DEFAULT_PERMISSION_MODULES);
        const condo = await assertOwnerCondo(ownerId, condoId);
        if (!condo)
            return res.status(403).json({ error: "Forbidden (not your condo)" });
...
router.post("/condos/:condoId/staff", async (req, res) => {
    ...
    const allowedModules = allowedModulesRaw
        .map((m) => String(m))
        .filter((m) => DEFAULT_PERMISSION_MODULES.includes(m));
    if (!fullName)
        return res.status(400).json({ error: "fullName is required" });
    if (!email)
        return res.status(400).json({ error: "email is required" });
    if (!staffPosition)
        return res.status(400).json({ error: "staffPosition is required" });
    try {
        await ensurePermissionCatalog(DEFAULT_PERMISSION_MODULES);
        const condo = await assertOwnerCondo(ownerId, condoId);
        if (!condo)
...
router.patch("/condos/:condoId/staff/:membershipId", async (req, res) => {
    ...
    try {
        ...
        if (!membership)
            return res.status(404).json({ error: "Staff membership not found" });
        const dataMem = {};
        if ("staffPosition" in body)
            dataMem.staffPosition = body.staffPosition ? String(body.staffPosition).trim() : null;
        if ("isActive" in body)
            dataMem.isActive = Boolean(body.isActive);
        if (Object.keys(dataMem).length)
            await prisma.staffMembership.update({ where: { id: membershipId }, data: dataMem });
        const dataUser = {};
        if ("fullName" in body)
            dataUser.name = body.fullName ? String(body.fullName).trim() : undefined;
        if ("email" in body)
            dataUser.email = body.email ? String(body.email).trim().toLowerCase() : undefined;
        if ("phone" in body)
            dataUser.phone = body.phone ? String(body.phone).trim() : undefined;
        if (Object.keys(dataUser).length)
            await prisma.user.update({ where: { id: membership.staffUserId }, data: dataUser });
...
router.post("/rooms/:roomId/advance-payments", async (req, res) => {
    try {
        const ownerId = req.user?.id;
        const room = await assertOwnerRoomOrThrow(req, String(req.params.roomId));
