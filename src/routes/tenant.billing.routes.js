...
function mapTenantBillStatus(args) {
    ...
    if (latestTxn && latestTxn !== "CONFIRMED")
        return "PENDING_REVIEW";
    return "UNPAID";
}
function mapBillItemKey(itemType, itemName) {
    const type = String(itemType || "").toUpperCase();
    const name = String(itemName || "").toLowerCase();
    if (type === "RENT" || name.includes("เช่า"))
        return "rent";
    if (type === "WATER" || name.includes("น้ำ"))
        return "water";
    if (type === "ELECTRIC" || name.includes("ไฟ"))
        return "electricity";
    if (type === "CHARGE" || type === "EXTRA" || name.includes("ส่วนกลาง"))
        return "commonFee";
    return "other";
}
async function getResidencyByLineUserIdOrThrow(lineUserId) {
    const lineAcc = await prisma.lineAccount.findUnique({
        where: { lineUserId },
        select: { userId: true },
    });
    if (!lineAcc?.userId) {
        const err = new Error("ไม่พบบัญชี LINE นี้ในระบบ");
        err.status = 404;
        throw err;
    }
    ...
    if (!residency) {
        const err = new Error("ไม่พบข้อมูลการเข้าพัก");
        err.status = 404;
        throw err;
    }
    return residency;
            ...
            history: invoices.map((invoice) => {
                ...
                return {
                    ...
                    status: mapTenantBillStatus({
                        ...
                        ? "PAID"
                        : "PENDING_REVIEW",
                    paidAtISO: latestTxn?.paidAt?.toISOString?.() ??
