...
router.post("/create", async (req, res) => {
    try {
        const { lineUserId, problem_type, description, room, location, images } = req.body;
        ...
        if (!userId)
            return res.status(400).json({ error: "Authentication required (lineUserId or token)" });
        ...
        if (!residency)
            return res.status(400).json({ error: "ไม่พบข้อมูลห้องพัก กรุณาลงทะเบียนก่อน" });
...
router.get("/:id", async (req, res) => {
    try {
        ...
        return res.json({
            ...
            images: repair.attachments?.map((a) => a.fileUrl) || [],
        });
    }
    catch (err) {
        ...
        try {
            ...
            if (repairForNotify?.tenantUserId) {
                ...
                if (lineAccount?.lineUserId) {
                    ...
                    const msg = `🔔 แจ้งเตือนงานแจ้งซ่อม\n` +
