...
function parsePaidAt(raw: unknown) {
...
function mapPaymentMethod(raw: unknown): PaymentMethod {
  ...
  if (s === "CASH" || s === "เงินสด") return PaymentMethod.CASH;
  if (s === "TRANSFER" || s === "เงินโอน") return PaymentMethod.TRANSFER;
  if (s === "PROMPTPAY" || s === "พร้อมเพย์") return PaymentMethod.PROMPTPAY;
  ...
  try {
    ...
    const result = await prisma.$transaction(async (tx) => {
      ...
      const nextInvoiceStatus =
        paidTotal >= invoiceTotal ? InvoiceStatus.PAID : InvoiceStatus.ISSUED;
