import { Prisma, InvoiceStatus, ContractStatus, } from "@prisma/client";
...
function safeInvoiceStatus(raw) {
...
function buildDueDate(billingMonth, dueDay) {
...
function makeInvoiceNo(roomNo, billingMonth) {
...
async function findCycleForMonth(condoId, month) {
 ...
 */
async function getRentForRoom(room) {
...
async function getRoomExtraServiceItems(roomId) {
    ...
    return rows.map((row) => {
        const amount = row.template?.defaultAmount != null
...
async function getRoomChargeItems(roomId) {
    ...
    return rows.map((row) => ({
        ...
        amount: row.amountOverride != null
            ? toNumber(row.amountOverride)
            : toNumber(row.condoCharge.amount),
        condoChargeId: row.condoChargeId,
    }));
}
async function getMeterItemsForRoom(roomId, cycleId, requireMeter = false) {
    if (!cycleId) {
        if (requireMeter) {
            const err = new Error("ยังไม่พบรอบมิเตอร์ของเดือนนี้");
            err.status = 400;
            throw err;
        }
    ...
    if (!reading) {
        if (requireMeter) {
            const err = new Error("ยังไม่จดมิเตอร์ครบทุกห้อง");
            err.status = 400;
            throw err;
        }
        return { items: [], meterReadingId: null };
...
/* =========================
   PREVIEW GENERATE
========================= */
router.get("/condos/:condoId/invoices/generate-preview", async (req, res) => {
    try {
...
/* =========================
   GENERATE INVOICES
========================= */
router.post("/condos/:condoId/invoices/generate", async (req, res) => {
    try {
        const ownerId = req.user?.id;
        if (!ownerId) {
            return res.status(401).json({ error: "Unauthorized" });
        ...
        const condoId = String(req.params.condoId);
        await assertOwnerCondoOrThrow(ownerId, condoId);
        const { month, roomIds, requireMeter = false, overwriteDraft = true, status = "DRAFT", } = req.body ?? {};
        const billingMonth = parseMonthInput(month);
        const cycle = await findCycleForMonth(condoId, billingMonth);
        const invoiceStatus = safeInvoiceStatus(status);
        const roomIdList = Array.isArray(roomIds) && roomIds.length > 0
            ? roomIds.map((x) => String(x))
            : null;
        const rooms = await prisma.room.findMany({
            where: {
                condoId,
        ...
        for (const room of rooms) {
            ...
            const saved = await prisma.$transaction(async (tx) => {
                ...
                if (existing && existing.status !== InvoiceStatus.DRAFT && !overwriteDraft) {
                    const err = new Error(`ห้อง ${room.roomNo} มีใบแจ้งหนี้ที่ไม่ใช่ DRAFT อยู่แล้ว`);
                    err.status = 400;
                    throw err;
                }
                let invoiceId;
