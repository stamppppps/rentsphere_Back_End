...
let meterChargeColumnsAvailable = null;
let utilityMinimumChargeColumnAvailable = null;
...
function meterReadingSelect(withCharges) {
    return {
        id: true,
        condoId: true,
        roomId: true,
...
function condoUtilitySettingSelect(withMinimumCharge) {
    return {
        utilityType: true,
        billingType: true,
        rate: true,
...
function calcUtilityCharge(units, billingType, rate, minimumCharge) {
    if (billingType === "FLAT")
        return rate;
...
async function findOrCreateCycleForMonth(condoId, targetMonth) {
...
async function findPreviousReading(roomId, currentCycleMonth) {
    ...
    try {
        ...
        const saved = await prisma.$transaction(async (tx) => {
            ...
            if (safeCurrWater < prevWater) {
                const err = new Error("เลขมิเตอร์น้ำต้องไม่น้อยกว่าครั้งก่อน");
                err.status = 400;
                throw err;
            }
            if (safeCurrElectric < prevElectric) {
                const err = new Error("เลขมิเตอร์ไฟต้องไม่น้อยกว่าครั้งก่อน");
                err.status = 400;
                throw err;
            }
            ...
            const waterCharge = waterSetting
                ? calcUtilityCharge(waterUnits, waterSetting.billingType, Number(waterSetting.rate), withMinimumCharge
            ...
            const electricCharge = electricSetting
                ? calcUtilityCharge(electricUnits, electricSetting.billingType, Number(electricSetting.rate), withMinimumCharge
